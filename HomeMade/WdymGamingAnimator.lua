...
local function setupDesync(character)
    if not character then return end
    if characterClone then
        disableDesync()
    end
    hrp0 = character:WaitForChild("HumanoidRootPart", 10)
    if not hrp0 then return end
    if hrp1 then pcall(function() hrp1:Destroy() end) end
    if characterClone then pcall(function() characterClone:Destroy() end) end
    hrp1 = hrp0:Clone()
    ...
    characterClone.Name = "DesyncClone"
    local cloneHRP = characterClone:FindFirstChild("HumanoidRootPart")
    if cloneHRP then
        characterClone.PrimaryPart = cloneHRP
    ...
    for _, v in ipairs(characterClone:GetDescendants()) do
        if v:IsA("LuaSourceContainer") or v:IsA("LocalScript") or v:IsA("Script") then
            v:Destroy()
...
local function disableDesync()
    pcall(function()
        if characterClone and c then
            for _, part in ipairs(c:GetDescendants()) do
                if part:IsA("BasePart") then
                    local clonePart = getRelativeClonePart(part, c, characterClone)
        ...
        if hrp0 and c and c.Parent then
            hrp0.Parent = c
            if hrp0:FindFirstChild("RootJoint") then
                hrp0.RootJoint.Part0 = hrp0
            end
            if hrp1 then hrp1:Destroy() hrp1 = nil end
            for _, part in ipairs(c:GetDescendants()) do
                if part:IsA("BasePart") then
                    part.Transparency = 0
                end
        ...
        currentOrientation = Vector3.new(0,0,0)
...
local function syncPose(source, target)
    if not source or not target then return end
    for _, motor in ipairs(source:GetDescendants()) do
        if motor:IsA("Motor6D") then
            local targetMotor = target:FindFirstChild(motor.Name, true)
            if targetMotor and targetMotor:IsA("Motor6D") then
                targetMotor.C0 = motor.C0
                targetMotor.C1 = motor.C1
...
local function getRelativeClonePart(part, originalCharacter, cloneCharacter)
    local success, relativePath = pcall(function()
        return part:GetFullName():sub(#originalCharacter:GetFullName() + 2)
    end)
    if not success or not relativePath then return nil end
    local current = cloneCharacter
...
task.spawn(function()
    while isRunning do
        h:Wait()
        if desyncEnabled and c and c.Parent and hrp0 and hrp0.Parent and hrp1 and hrp1.Parent and characterClone then
            pcall(function()
                local targetCFrame = hrp1.CFrame * CFrame.new(-currentPosition) * CFrame.Angles(math.rad(currentOrientation.X), math.rad(currentOrientation.Y), math.rad(currentOrientation.Z))
                hrp0.CFrame = targetCFrame
                hrp0.Velocity = hrp1.Velocity
                local desyncTransform = targetCFrame * hrp1.CFrame:Inverse()
                for _, part in ipairs(c:GetDescendants()) do
                    if part:IsA("BasePart") and part.Name ~= "HumanoidRootPart" then
                        local clonePart = getRelativeClonePart(part, c, characterClone)
                        if clonePart and clonePart:IsA("BasePart") then
                            clonePart.CFrame = desyncTransform * part.CFrame
                        end
                    end
                end
                local cloneHRP = characterClone:FindFirstChild("HumanoidRootPart")
...
MainFrame.Size = UDim2.new(0, 250, 0, 395)
...
local OrientationInput = Instance.new("TextBox")
...
local DesyncToggleBtn = Instance.new("TextButton")
...
DesyncToggleBtn.Size = UDim2.new(1, -24, 0, 28)
DesyncToggleBtn.Font = Enum.Font.SourceSansBold
DesyncToggleBtn.TextSize = 16
...
DesyncToggleBtn.MouseButton1Click:Connect(function()
    ...
    if desyncEnabled then
        ...
        setupDesync(lp.Character or lp.CharacterAdded:Wait())
    else
        DesyncToggleBtn.Text = "Desync: OFF"
        DesyncToggleBtn.TextColor3 = Color3.fromRGB(255, 100, 100)
        disableDesync()
...
ApplyBtn.MouseButton1Click:Connect(function()
    currentOrientation = stringToVector3(OrientationInput.Text)
    currentPosition = stringToVector3(PositionInput.Text)
    ...
    desyncAnimThread = task.spawn(function()
        while currentDesyncPlaying and isRunning do
            for _, frame in ipairs(frames) do
                ...
                local startRot = currentOrientation
                local startPos = currentPosition
                local elapsed = 0
                while elapsed < duration and currentDesyncPlaying and isRunning do
                    local step = h:Wait()
                    elapsed = elapsed + step
                    local t = math.min(elapsed / duration, 1)
                    currentOrientation = startRot:Lerp(targetRot, t)
                    currentPosition = startPos:Lerp(targetPos, t)
                end
            end
...
-- ==========================================
-- ИСПРАВЛЕННАЯ СЕКЦИЯ AnimEditorBtn
-- ==========================================
AnimEditorBtn.MouseButton1Click:Connect(function()
    ...
    -- Функция обновления списка кадров (определена до использования)
    ...
    -- Функция обновления выпадающего списка загрузки
        ...
        for name, data in pairs(savedDesyncAnimations) do
            if data.frames then
                ...
                b.MouseButton1Click:Connect(function()
                    ...
                    print("Frames count:", loaded and loaded.frames and #loaded.frames or 0)
                    if loaded and loaded.frames and #loaded.frames > 0 then
                        currentEditingAnimation = {}
                        for _, frame in ipairs(loaded.frames) do
                            print("Frame data:", frame.rot, frame.pos, frame.time)
                        ...
                        print("Successfully loaded", #currentEditingAnimation, "frames into list")
                    else
...
NormalAnimationsBtn.MouseButton1Click:Connect(function()
    ...
    local CatToggleFrame = Instance.new("Frame")
    CatToggleFrame.Size = UDim2.new(1, -20, 0, 25)
    CatToggleFrame.Position = UDim2.new(0, 10, 0, 5)
    CatToggleFrame.BackgroundTransparency = 1
    CatToggleFrame.Parent = ContainerA

    local R6Btn = Instance.new("TextButton")
    R6Btn.Text = "R6"
    R6Btn.Size = UDim2.new(0.5, -2, 1, 0)
    ...
    R6Btn.Parent = CatToggleFrame

    local R15Btn = Instance.new("TextButton")
    R15Btn.Text = "R15"
    R15Btn.Size = UDim2.new(0.5, -2, 1, 0)
    R15Btn.Position = UDim2.new(0.5, 2, 0, 0)
    ...
    R15Btn.Parent = CatToggleFrame

    local SubCatContainer = Instance.new("Frame")
    SubCatContainer.Size = UDim2.new(1, -20, 0, 24)
    SubCatContainer.Position = UDim2.new(0, 10, 0, 35)
    SubCatContainer.BackgroundTransparency = 1
    SubCatContainer.Parent = ContainerA

    local SubCatDropdown = Instance.new("TextButton")
    SubCatDropdown.Text = "Category: [Main] v"
    SubCatDropdown.Size = UDim2.new(1, -75, 1, 0)
    ...
    SubCatDropdown.Parent = SubCatContainer

    local DelSubCatBtn = Instance.new("TextButton")
    DelSubCatBtn.Text = "Delete Sub"
    DelSubCatBtn.Size = UDim2.new(0, 70, 1, 0)
    DelSubCatBtn.Position = UDim2.new(1, -70, 0, 0)
    ...
    DelSubCatBtn.Parent = SubCatContainer

    local SubDropdownList = Instance.new("ScrollingFrame")
    SubDropdownList.Size = UDim2.new(1, -20, 0, 100)
    SubDropdownList.Position = UDim2.new(0, 10, 0, 60)
    ...
    SearchBar.PlaceholderText = "Search animation..."
    SearchBar.Text = ""
    ...
    SearchBar.PlaceholderColor3 = Color3.fromRGB(140, 100, 140)
    SearchBar.Font = Enum.Font.SourceSansItalic
    ...
    AnimListFrame.Size = UDim2.new(1, -20, 0, 130)
    AnimListFrame.Position = UDim2.new(0, 10, 0, 95)
    AnimListFrame.BackgroundColor3 = Color3.fromRGB(77, 34, 77)
    ...
    local AnimListLayout = Instance.new("UIListLayout", AnimListFrame)
    AnimListLayout.Padding = UDim.new(0, 2)

    local function createBottomInput(placeholder, yPos, xSizeOffset)
        local box = Instance.new("TextBox")
        box.PlaceholderText = placeholder
        box.Text = ""
        box.Size = UDim2.new(1, xSizeOffset or -20, 0, 24)
        box.Position = UDim2.new(0, 10, 0, yPos)
        box.BackgroundColor3 = Color3.fromRGB(77, 34, 77)
        box.TextColor3 = Color3.fromRGB(255, 255, 255)
        box.PlaceholderColor3 = Color3.fromRGB(140, 100, 140)
        box.Font = Enum.Font.SourceSans
        box.TextSize = 14
        box.ClearTextOnFocus = false
        Instance.new("UICorner", box).CornerRadius = UDim.new(0, 4)
        box.Parent = ContainerA
        return box
    end

    local NewSubInput = createBottomInput("New subcategory name...", 235, -90)
    local AddSubBtn = Instance.new("TextButton")
    AddSubBtn.Text = "Create Sub"
    AddSubBtn.Size = UDim2.new(0, 75, 0, 24)
    AddSubBtn.Position = UDim2.new(1, -85, 0, 235)
    AddSubBtn.BackgroundColor3 = Color3.fromRGB(90, 40, 90)
    ...
    local AnimNameIn = createBottomInput("Animation Name...", 265)
    local AnimIdIn = createBottomInput("Animation ID (rbxassetid://...)", 295)

    local SettingsPanel = Instance.new("Frame")
    SettingsPanel.Size = UDim2.new(1, -20, 0, 55)
    SettingsPanel.Position = UDim2.new(0, 10, 0, 325)
    ...
    ActionFrame.Position = UDim2.new(0, 10, 0, 390)
    ...
    updateAnimsList = function()
        ...
        if currentSubCat == "[Main]" then
            ...
            DelSubCatBtn.Visible = false
            SubCatDropdown.Size = UDim2.new(1, 0, 1, 0)
        else
...
if lp.Character then initCharacter(lp.Character) end
lp.CharacterAdded:Connect(function(newCharacter)
    c = newCharacter
    animationPlaying = false
    if desyncEnabled then setupDesync(newCharacter) end
...
MinimizeBtn.MouseButton1Click:Connect(function()
    ...
    if minimized then
        ...
        MainFrame:TweenSize(UDim2.new(0, 250, 0, 395), "InOut", "Quad", 0.2, true)
        MinimizeBtn.Text = "-"
    end
end)
