...
function Library:Notify(title, text, duration)
    ...
    local main = CreateInstance("Frame", {
        ...
        BackgroundColor3 = self.Theme.MainWindow,
    ...
    table.insert(Notifications, gui)
    return gui
end
...
function Library:CreateWindow(settings)
    ...
    local function CreateTopBtn(symbol, offset)
        ...
        btn.MouseLeave:Connect(function()
            btn.BackgroundColor3 = self.Theme.TopBar
        end)
    ...
    -- Menu Sidebar (for tabs)
    self.MenuSidebar = CreateInstance("ScrollingFrame", {
        Name = "MenuSidebar",
        Parent = self.Window,
    ...
    minimizeBtn.MouseButton1Click:Connect(function()
        if state == "full" then
            ...
            state = "full"
            local tween = TweenService:Create(self.Window, TweenInfo.new(0.22, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = fullSize})
            tween:Play()
        end
    end)
    ...
    expandBtn.MouseButton1Click:Connect(function()
        if state == "full" then
            state = "strip"
            local tween = TweenService:Create(self.Window, TweenInfo.new(0.22, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = stripSize})
            tween:Play()
...
function Library:CreateSidebarButton(text, callback)
    ...
    btn.MouseLeave:Connect(function()
        btn.BackgroundColor3 = self.Theme.SideBar
    end)
...
function Library:CreateToggle(tab, labelText, default, callback)
    ...
    local function updateText()
        ...
        btn.BackgroundColor3 = state and 
            Color3.fromRGB(100, 40, 40) or 
            Color3.fromRGB(40, 100, 40)
        btn.TextColor3 = state and 
            self.Theme.ToggleOff or 
            self.Theme.ToggleOn
    end
...
function Library:CreateDropdown(tab, labelText, options, default, callback)
    ...
    list:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        list.CanvasSize = UDim2.new(0, 0, 0, list.AbsoluteContentSize.Y + 4)
    end)
...
function Library:CreateKeySystem(settings)
    ...
    local keyTopBar = CreateInstance("Frame", {
        Parent = keyWindow,
    ...
    local function checkKey()
        ...
        if keyUrl ~= "" then
            ...
            if success then
                -- Simple key validation (you can enhance this)
                if response:find(enteredKey) then
                    keyWindow:Destroy()
                    onSuccess()
                else
                ...
                infoLabel.Text = "Failed to fetch key database!"
                infoLabel.TextColor3 = Color3.fromRGB(255, 100, 100)
            end
        else
            -- Local key check
    ...
    local submitBtn = CreateInstance("TextButton", {
        ...
        Position = UDim2.new(0.5, -75, 0, 240),
        BackgroundColor3 = Color3.fromRGB(40, 90, 40),
        BorderColor3 = self.Theme.Border,
        TextColor3 = Color3.fromRGB(255, 255, 255),
        Text = "Check Key",
        Font = self.Font,
...
function Library:CreateSettingsTab()
    ...
    -- Color settings
    local colors = {
        {name = "Main Window", key = "MainWindow"},
        {name = "Top Bar", key = "TopBar"},
        {name = "Side Bar", key = "SideBar"},
        {name = "Text", key = "Text"},
