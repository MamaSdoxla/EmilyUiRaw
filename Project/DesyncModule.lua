...
-- DesyncModule.lua
-- ============================================================
-- Модуль Desync: анимации, редактор, менеджер, логгер, кейбинды.
...
local function loadKeys()
    ...
    if b.Animations and not b.Animations[1] then
        ...
        if old.Desync and old.Desync.key then
            table.insert(b.Animations, {key = old.Desync.key, type = "Desync", animName = old.Desync.animName, speed = old.Desync.speed or 1, looped = old.Desync.looped or false, reversed = old.Desync.reversed or false})
        end
        if old.Normal and old.Normal.key then
            table.insert(b.Animations, {key = old.Normal.key, type = "Normal", animName = old.Normal.animName, bodyType = old.Normal.bodyType or "R6", speed = old.Normal.speed or 1, looped = old.Normal.looped or false, reversed = old.Normal.reversed or false})
        end
    end
    return b
end
...
local function stopDesync()
    ...
    if DesyncToggleBtn then
        ...
        FuckYouLib.paintToggleBtn(DesyncToggleBtn, false)
    end
...
local function startDesync()
    ...
    IsDesynced = true
    if DesyncToggleBtn then DesyncToggleBtn.Text = "Desync: ON"; FuckYouLib.paintToggleBtn(DesyncToggleBtn, true) end
    realCF = hrp.CFrame
...
local function playNormAnim(bt, name, speed, looped, reversed)
    ...
    if not id then
        for _, st in pairs(data._subcategories or {}) do
            if st[name] then id = st[name]; break end
        end
    end
...
local function handleKeybind(key)
    if not key or not DesyncSectionEnabled then return end
...
local function mkLabel(p, txt, sz, pos, font, ts)
    ...
    l.Font = font or F_S; l.TextSize = 13; l.TextXAlignment = Enum.TextXAlignment.Left
    l.TextColor3 = FuckYouLib.uiColor_TextColor; l.BackgroundTransparency = 1
...
local function mkBox(p, txt, sz, pos, ts)
    ...
    b.Font = F_R; b.TextSize = ts or 16
...
local function mkListBG(p, sz, pos)
    local f = FuckYouLib.create("ScrollingFrame", {
        Parent = p, Size = sz, Position = pos,
...
local function trackOpacity(inst)
    inst.BackgroundTransparency = 1 - FuckYouLib.uiGuiOpacity
...
local function buildDesyncTab(parent)
    local sf = FuckYouLib.create("ScrollingFrame", {Parent = parent, Size = UDim2.new(1,0,1,0), BackgroundTransparency = 1, BorderSizePixel = 0, ScrollBarThickness = 4, CanvasSize = UDim2.new(0,0,0,260)})
    ...
    FuckYouLib.registerToggle(DesyncToggleBtn, function() return IsDesynced end)
...
local function buildEditorTab(parent)
    local sf = FuckYouLib.create("ScrollingFrame", {Parent = parent, Size = UDim2.new(1,0,1,0), BackgroundTransparency = 1, BorderSizePixel = 0, ScrollBarThickness = 4, CanvasSize = UDim2.new(0,0,0,280)})
    local inner = mkPanel(sf, UDim2.new(1,0,0,280))
    local left = FuckYouLib.create("Frame", {Parent = inner, Size = UDim2.new(0.5,-4,1,0), Position = UDim2.new(0,2,0,0), BackgroundTransparency = 1})
    local right = mkListBG(inner, UDim2.new(0.5,-6,1,-10), UDim2.new(0.5,4,0,5))
    local rLayout = FuckYouLib.create("UIListLayout", {Parent = right, Padding = UDim.new(0,2)})
    rLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() right.CanvasSize = UDim2.new(0,0,0, rLayout.AbsoluteContentSize.Y + 6) end)
    ...
    loadDD.ZIndex = 5
    FuckYouLib.create("UIListLayout", {Parent = loadDD})
    ...
    local function refresh()
        ...
        for i, fd in ipairs(editing) do
            local b = FuckYouLib.create("TextButton", {Parent = right, Size = UDim2.new(1,0,0,20), BackgroundColor3 = (selIdx == i) and lighter(FuckYouLib.uiColor_ButtonColor,40) or FuckYouLib.uiColor_ButtonColor, TextColor3 = FuckYouLib.uiColor_TextColor, Text = string.format("[%d] P:%s O:%s T:%s", i, fd.pos, fd.rot, tostring(fd.time)), Font = F_R, TextSize = 13, BorderSizePixel = 0})
            corner(b)
            trackOpacity(b)
...
local function buildDesyncAnimsTab(parent)
    ...
    ddList.ZIndex = 5
    FuckYouLib.create("UIListLayout", {Parent = ddList})
    local selected = nil
    ...
    loopBox.MouseButton1Click:Connect(function()
        ...
        loopBox.TextColor3 = FuckYouLib.uiColor_TextColor
    end)
...
local function buildManagerTab(parent)
    local sf = FuckYouLib.create("ScrollingFrame", {Parent = parent, Size = UDim2.new(1,0,1,0), BackgroundTransparency = 1, BorderSizePixel = 0, ScrollBarThickness = 4, CanvasSize = UDim2.new(0,0,0,500)})
    ...
    subList.ZIndex = 10; subList.Visible = false
    local subListLayout = FuckYouLib.create("UIListLayout", {Parent = subList})
    subListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() subList.CanvasSize = UDim2.new(0,0,0, subListLayout.AbsoluteContentSize.Y + 4) end)
    local delSub = mkBtn(inner, "Del Sub", UDim2.new(0,70,0,24), UDim2.new(1,-80,0,40), F_B, 12, "no")
    delSub.BackgroundColor3 = C_REDD; delSub.TextColor3 = C_WHT; delSub.Visible = false
    ...
    FuckYouLib.create("UIListLayout", {Parent = list, Padding = UDim.new(0,2)})
    ...
    spdIn.FocusLost:Connect(function() spd = tonumber(spdIn.Text) or 1; if activeTrack and activeTrack.IsPlaying then activeTrack:AdjustSpeed(spd * (reversed and -1 or 1)) end end)
    ...
    updateList = function()
        ...
        for name, id in pairs(target) do
            if name ~= "_subcategories" and (search == "" or string.find(string.lower(name), string.lower(search), 1, true)) then
                c = c + 1
                local row = FuckYouLib.create("Frame", {Parent = list, Size = UDim2.new(1,0,0,26), BackgroundTransparency = 1})
                ...
                db.BackgroundColor3 = C_REDD; db.TextColor3 = C_WHT
                pb.MouseButton1Click:Connect(function()
                    ...
                    if desyncChar and desyncChar:FindFirstChildOfClass("Humanoid") then
                        ...
                        if ok and tr then
                            ...
                            if reversed then tr.TimePosition = tr.Length > 0 and tr.Length or 0.1 end
                        end
                    end
                end)
    ...
    local function selectCat(c2)
        ...
        if c2 == "R6" then
            r6B.BackgroundColor3 = FuckYouLib.uiColor_ButtonColor; r6B.TextColor3 = Color3.fromRGB(255,255,255)
            r15B.BackgroundColor3 = darker(FuckYouLib.uiColor_ButtonColor,15); r15B.TextColor3 = FuckYouLib.uiColor_TextColor
        else
            r15B.BackgroundColor3 = FuckYouLib.uiColor_ButtonColor; r15B.TextColor3 = Color3.fromRGB(255,255,255)
            r6B.BackgroundColor3 = darker(FuckYouLib.uiColor_ButtonColor,15); r6B.TextColor3 = FuckYouLib.uiColor_TextColor
        end
    ...
    delSub.MouseButton1Click:Connect(function()
        if sub ~= "[Main]" then
    ...
    addAnim.MouseButton1Click:Connect(function()
        ...
        if n ~= "" and id ~= "" then
            ...
            local d = managerDataCache[cat]
            if sub == "[Main]" then d[n] = clean
            else d._subcategories[sub][n] = clean end
            saveMgr(cat, d)
            nameIn.Text = ""; idIn.Text = ""
            updateList()
        end
    end)
...
local function buildKeybindsTab(parent)
    local sf = FuckYouLib.create("ScrollingFrame", {Parent = parent, Size = UDim2.new(1,0,1,0), BackgroundTransparency = 1, BorderSizePixel = 0, ScrollBarThickness = 4, CanvasSize = UDim2.new(0,0,0,400)})
    ...
    typeList.ZIndex = 15; typeList.Visible = false
    FuckYouLib.create("UIListLayout", {Parent = typeList})
    local refresh
    ...
    typeDD.MouseButton1Click:Connect(function() typeList.Visible = not typeList.Visible end)
    ...
    refresh = function()
        ...
        if curType == "Animations" then
            ...
            atList.ZIndex = 10; atList.Visible = false
            FuckYouLib.create("UIListLayout", {Parent = atList})
            ...
            if curAnimType == "Normal" then
                ...
                local btDD = mkBtn(inner, curBody .. " v", UDim2.new(0,120,0,24), UDim2.new(0,140,0,y-2), F_R, 16)
                local btList = mkListBG(inner, UDim2.new(0,120,0,50), UDim2.new(0,140,0,y+24))
                btList.ZIndex = 10; btList.Visible = false
                FuckYouLib.create("UIListLayout", {Parent = btList})
                for _, t in ipairs({"R6", "R15"}) do
                    ...
                    b.MouseButton1Click:Connect(function()
                        ...
                        selNormal = nil
                        if selLbl then selLbl.Text = "Selected: none" end
                        if updateDL and sBox then updateDL(sBox.Text) end
                    end)
                end
                btDD.MouseButton1Click:Connect(function() btList.Visible = not btList.Visible end)
                y = y + 32
            end
            ...
            FuckYouLib.create("UIListLayout", {Parent = aList, Padding = UDim.new(0,2)})
            updateDL = function(filter)
                for _, ch in ipairs(aList:GetChildren()) do if ch:IsA("TextButton") then ch:Destroy() end end
                local c = 0
                local pool = {}
                if curAnimType == "Desync" then
                    for n, d in pairs(savedDesyncAnimations) do if d.frames then pool[n] = true end end
                else
                    local data = managerDataCache[curBody] or {}
                    for n, _ in pairs(data) do if n ~= "_subcategories" then pool[n] = true end end
                    for _, st in pairs(data._subcategories or {}) do for n, _ in pairs(st) do pool[n] = true end end
                end
                for n, _ in pairs(pool) do
                    if filter == "" or string.find(string.lower(n), string.lower(filter), 1, true) then
                        ...
                        local b = FuckYouLib.create("TextButton", {Parent = aList, Size = UDim2.new(1,0,0,22), BackgroundColor3 = isSel and lighter(FuckYouLib.uiColor_ButtonColor,40) or FuckYouLib.uiColor_ButtonColor, TextColor3 = FuckYouLib.uiColor_TextColor, Text = n, Font = F_R, TextSize = 13, BorderSizePixel = 0})
                        corner(b); trackOpacity(b)
                        b.TextXAlignment = Enum.TextXAlignment.Left
                        b.MouseButton1Click:Connect(function()
                            if curAnimType == "Desync" then selDesync = n
                            else selNormal = n end
                            updateDL(sBox.Text)
            ...
            y = y + 120
            if curAnimType == "Desync" then
                bindRow(y, function() return tempDesync end, function(v) tempDesync = v end, function(saveB)
                ...
                bindRow(y, function() return tempNormal end, function(v) tempNormal = v end, function(saveB)
        ...
        elseif curType == "View all keybinds" then
            ...
            FuckYouLib.create("UIListLayout", {Parent = scroll, Padding = UDim.new(0,4)})
            local all = {}
            if keybinds.Desync and keybinds.Desync.key then table.insert(all, {type = "Desync", key = keybinds.Desync.key, path = {"Desync"}, data = keybinds.Desync}) end
            if keybinds.Menu and keybinds.Menu.key then table.insert(all, {type = "Menu", key = keybinds.Menu.key, path = {"Menu"}, data = keybinds.Menu}) end
            ...
            for _, kb in ipairs(all) do
                ...
                local row = FuckYouLib.create("Frame", {Parent = scroll, Size = UDim2.new(1,-10,0,rh), BackgroundColor3 = FuckYouLib.uiColor_ButtonColor, BorderSizePixel = 0})
                table.insert(FuckYouLib.themeElements.Buttons, row)
                corner(row)
                local desc = string.format("%s [%s]", kb.type, kb.key)
                if kb.animName then desc = desc .. " -> " .. kb.animName .. (kb.bodyType and (" (" .. kb.bodyType .. ")") or "") end
                ...
                del.BackgroundColor3 = C_REDD; del.TextColor3 = C_WHT
                del.MouseButton1Click:Connect(function()
                    local t = keybinds
                    for i = 1, #kb.path - 1 do t = t[kb.path[i]] end
                    local lk = kb.path[#kb.path]
                    if type(lk) == "number" then table.remove(t, lk)
                    else t[lk] = nil end
                    saveKeys(keybinds)
                    refresh()
                end)
                if kb.type:find("Anim") then
                    ...
                    si.FocusLost:Connect(function()
                        ...
                        if v then kb.data.speed = v; saveKeys(keybinds) end
                    end)
...
local function trackAnimator(an)
    an.AnimationPlayed:Connect(function(tr)
        if not isRunning or not DesyncSectionEnabled then return end
        local id, nm = nil, nil
...
local function buildLoggerTab(parent)
    local sf = FuckYouLib.create("ScrollingFrame", {Parent = parent, Size = UDim2.new(1,0,1,0), BackgroundTransparency = 1, BorderSizePixel = 0, ScrollBarThickness = 4, CanvasSize = UDim2.new(0,0,0,300)})
    ...
    local lay = FuckYouLib.create("UIListLayout", {Parent = list, Padding = UDim.new(0,2)})
    lay:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() list.CanvasSize = UDim2.new(0,0,0, lay.AbsoluteContentSize.Y + 4) end)
    refreshLoggerList = function()
        ...
        for _, e in ipairs(loggedAnims) do
            ...
            b.MouseButton1Click:Connect(function()
                ...
                FuckYouLib.notify("Anim Logger", "Copied: " .. e.id)
            end)
        end
    end
    refreshLoggerList()
...
local function addDesyncTab(name, builder)
    ...
    builder(frame)
    local btn = FuckYouLib.create("TextButton", {Name = "DBtn_" .. name, Parent = FuckYouLib.MenuInsided, Size = UDim2.new(1,0,0,40), LayoutOrder = 100 + #desyncTabs, Visible = false, BackgroundColor3 = FuckYouLib.uiColor_ButtonColor, BorderColor3 = COL_BORDER, TextColor3 = FuckYouLib.uiColor_TextColor, Text = name, Font = FONT, TextSize = 12, TextWrapped = true})
...
local baseApplyThemeDesync = FuckYouLib.applyTheme
...
-- Toggle в сайдбаре для Desync
local DesyncSidebarToggle = FuckYouLib.create("TextButton", {
    Name = "MToggle_Desync", Parent = FuckYouLib.MenuInsided,
...
table.insert(FuckYouLib.themeElements.CustomButtons, DesyncSidebarToggle)
table.insert(FuckYouLib.moduleToggles, {btn = DesyncSidebarToggle, group = "Desync"})
FuckYouLib.registerToggle(DesyncSidebarToggle, function() return DesyncSectionEnabled end)
local function refreshDesyncToggleText()
    DesyncSidebarToggle.Text = "Desync: " .. (DesyncSectionEnabled and "ON" or "OFF")
    FuckYouLib.paintToggleBtn(DesyncSidebarToggle, DesyncSectionEnabled)
end
...
FuckYouLib.registerKeyListProvider("Desync", function()
    local rows = {}
