...
local function notify(title, text)
    task.spawn(function()
        local notificationData = {Title = title, Text = text, Duration = 5}
...
local function saveConfig()
    local config = {
        ...
        GuiOpacity = uiGuiOpacity, ImageOpacity = uiImageOpacity, Blur = uiBlurSize, Fit = uiFitMode, BackgroundFile = uiBackgroundFile,
    }
    ...
    if isfile and isfile(configPath) and readfile then
        ...
        if success and json then
            ...
            if ok and config then
                ...
                if config.ToggleOnColor then uiColor_ToggleOnText = Color3.new(unpack(config.ToggleOnColor)) end
                if config.ToggleOffColor then uiColor_ToggleOffText = Color3.new(unpack(config.ToggleOffColor)) end
...
local function makeTopBtn(symbol, offset)
    local b = create("TextButton", {
        Name = symbol, Parent = TopBar, Position = UDim2.new(1, -45 * offset, 0, 0), Size = UDim2.new(0, 45, 0, 45),
        BackgroundColor3 = uiColor_TopBar, BorderColor3 = COL_BORDER, Text = symbol, TextColor3 = uiColor_TextColor, TextSize = 13, Font = FONT, ZIndex = 3
    })
    table.insert(themeElements.TopBars, b)
    table.insert(themeElements.Texts, b)
    b.MouseEnter:Connect(function() b.BackgroundColor3 = Color3.fromRGB(math.min(b.BackgroundColor3.R*255+10,255), math.min(b.BackgroundColor3.G*255+10,255), math.min(b.BackgroundColor3.B*255+10,255)) end)
...
local function makeSideBtn(text, offsetY)
    local b = create("TextButton", {
        Name = text, Parent = SideBard, Position = UDim2.new(0, 0, 0, offsetY), Size = UDim2.new(1, 0, 0, 59),
        BackgroundColor3 = uiColor_SideBar, BorderColor3 = COL_BORDER, Text = text, TextColor3 = uiColor_TextColor, TextSize = 12, Font = FONT, ZIndex = 3
    })
...
local function createContentButton(parent, text, callback, customColor)
    ...
    btn.MouseEnter:Connect(function() btn.BackgroundColor3 = Color3.fromRGB(math.min(btn.BackgroundColor3.R*255+10,255), math.min(btn.BackgroundColor3.G*255+10,255), math.min(btn.BackgroundColor3.B*255+10,255)) end)
...
function(v) uiImageOpacity = v / 100; applyBackground() end,
...
createContentButton(tabFrames.Settings, "Refresh configlist", refreshConfigList)
...
createContentButton(tabFrames.Settings, "Reset defaults", function()
    ...
    uiColor_MainWindow = COL_BG; uiColor_TopBar = COL_BG; uiColor_SideBar = COL_BG
    uiColor_TextColor = COL_TEXT; uiColor_ButtonColor = COL_BG; uiColor_TextBoxColor = COL_TEXTBOX
    uiColor_ToggleOnText = Color3.fromRGB(100, 255, 100); uiColor_ToggleOffText = Color3.fromRGB(255, 100, 100)
    uiGuiOpacity = 1; uiImageOpacity = 1; uiBlurSize = 0; uiFitMode = "Fill"; uiBackgroundFile = ""
    applyBackground(); updateBlur(); keyBindBtn.Text = currentToggleKey.Name
...
local function isGroupAllowed(groupName)
    ...
    if beta then return g == "tester" or g == "coder"
    else return g == "free" or g == "user" or g == "tester" or g == "coder" end
end
