--// EmilyUiModule.lua — EmilyUi Module
--// Вкладки: Main, Universal, Character, Players, Visuals, Utilities, Server, Games, Scripts, Hubs, GUIs, Animations, Settings
...
local function initEmilyUiModule(Library)
    ...
    local function extraSlider(parent, labelText, min, max, decimals, default, getval, onval, fmt)
        ...
        local valLabel = Library.create("TextLabel", {
            Size = UDim2.new(rightEnd - 0.48, 0, 0, 14),
        ...
        local track = Library.create("TextButton", {
            Size = UDim2.new(rightEnd - 0.48, 0, 0, 10),
        ...
        local fill = Library.create("Frame", {
            Size = UDim2.new(math.clamp((getval() - min) / (max - min), 0, 1), 0, 1, 0),
        ...
        if default then
            local defBtn = Library.create("TextButton", {
                Size = UDim2.new(0.13, 0, 0.7, 0),
                Position = UDim2.new(0.86, 0, 0.15, 0),
                BackgroundColor3 = Library.uiColor_ButtonColor,
                BorderColor3 = Library.COL_BORDER,
                BackgroundTransparency = 1 - Library.uiGuiOpacity,
                TextColor3 = Library.uiColor_TextColor,
                Text = "Default", Font = Library.FONT, TextSize = 10,
                Parent = container
            })
            table.insert(Library.themeElements.Buttons, defBtn)
            table.insert(Library.themeElements.Texts, defBtn)
    ...
    local function extraDropdown(parent, labelText, options, getcur, onselect)
        ...
        table.insert(Library.themeElements.Texts, label)
        local btn = Library.createContentButton(container, labelText .. ": " .. getcur(), function() end)
    ...
    local UserKeyTimeLabel = Library.create("TextLabel", {
        Parent = UserProfilePanel,
    ...
    table.insert(Library.themeElements.Texts, UserKeyTimeLabel)
    local UserGroupLabel = Library.create("TextLabel", {
        Parent = UserProfilePanel,
    ...
    RunService.RenderStepped:Connect(function()
        if not Library.FuckYou.Visible then return end
        local group = Library.currentKeyData.group or "Free"
    ...
    Library.createContentButton(tabFrames.Universal, "Infinite Yield", function()
        loadstring(game:HttpGet('https://raw.githubusercontent.com/EdgeIY/infiniteyield/master/source'))()
    end)
    Library.createContentButton(tabFrames.Universal, "FE Admin Commands", function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/lxte/cmd/main/main.lua"))()
    end)
    Library.createSection(tabFrames.Universal, "For exploiting")
    Library.createContentButton(tabFrames.Universal, "Dex Explorer++", function()
        loadstring(game:HttpGet("https://github.com/AZYsGithub/DexPlusPlus/releases/latest/download/out.lua"))()
    end)
    Library.createContentButton(tabFrames.Universal, "Cobalt", function()
        loadstring(game:HttpGet("https://gitlab.com/upio/cobalt/-/releases/permalink/latest/downloads/Cobalt.luau"))()
    end)
    Library.createContentButton(tabFrames.Universal, "Rem v1.2", function()
        loadstring(game:HttpGet("https://e-vil.com/anbu/rem.lua"))()
    end)
    Library.createContentButton(tabFrames.Universal, "VEX (better DEX)", function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/Vezise/2026/main/Vez/VexExplorer/VEXExplorer.lua"))()
    end)
    Library.createContentButton(tabFrames.Universal, "Executor Tester | v2.6", function()
    ...
    local function updateViewButtons()
        for plr, cardData in pairs(playerCards) do
            if cardData.ViewBtn then
                cardData.ViewBtn.Text = (spectateTarget == plr) and "Stop" or "View"
            end
        end
    end
    ...
    local function gotoPlayer(plr)
        ...
        if r and t then
            ...
            Library.notify("Players", "Teleported to " .. plr.Name)
        end
    end
    ...
    Library.createContentButton(tabFrames.Players, "Refresh List", function() refreshPlayersList() end)
    Library.createContentButton(tabFrames.Players, "Unhide All", function()
        hiddenPlayers = {}
        refreshPlayersList()
    end)
    ...
    local function buildPlayerCard(plr)
        local card = Library.create("Frame", {
            Name = "PlayerCard", Parent = tabFrames.Players,
            LayoutOrder = 1, Size = UDim2.new(1, 0, 0, 120),
            BackgroundTransparency = 1, BorderSizePixel = 0
        })
        local header = Library.create("Frame", {
            Parent = card, Size = UDim2.new(1, 0, 0, 44),
            BackgroundTransparency = 1
        })
        Library.create("ImageLabel", {
            Parent = header,
        ...
        Library.create("TextLabel", {
            Parent = header,
            Position = UDim2.new(0, 48, 0, 3),
            Size = UDim2.new(0.55, -48, 0, 18),
            BackgroundTransparency = 1,
        ...
        local row = Library.create("Frame", {
            Parent = card, Size = UDim2.new(1, 0, 0, 28),
            BackgroundTransparency = 1
        })
        local function cardBtn(text, i)
            local b = Library.create("TextButton", {
                ...
                Position = UDim2.new(0.2 * i, 0, 0, 0),
                BackgroundColor3 = Library.uiColor_ButtonColor,
                BorderColor3 = Library.COL_BORDER,
                BackgroundTransparency = 1 - Library.uiGuiOpacity,
                TextColor3 = Library.uiColor_TextColor,
                Text = text, Font = Library.FONT,
                TextSize = 11, TextWrapped = true
            })
            table.insert(Library.themeElements.Buttons, b)
    ...
    local function setCrosshair(on)
        if on and not crosshairGui then
            crosshairGui = Library.create("ScreenGui", {
                Name = "FYCrosshair", ResetOnSpawn = false,
                IgnoreGuiInset = true,
            ...
            for _, g in ipairs(geo) do
                Library.create("Frame", {
                    ...
                    Size = UDim2.new(0, g[1], 0, g[2]),
                    Position = UDim2.new(0.5, g[3], 0.5, g[4]),
                    BackgroundColor3 = Library.uiColor_TextColor,
                    BorderSizePixel = 0
                })
            end
    ...
    ccEff.Saturation = 0; ccEff.Contrast = 0
    ...
    Library.createContentButton(tabFrames.Utilities, "Server Hop", function()
        Library.notify("Server Hop", "Searching...")
    end)
    Library.createContentButton(tabFrames.Utilities, "Rejoin", function()
    ...
    local srvUptimeL = Library.createLabel(tabFrames.Server, "Uptime: 00:00:00")
    ...
    --// ==================== GAMES / SCRIPTS / HUBS / GUIs / ANIMS ====================
    -- (упрощённо — аналогично оригиналу через uiStructure)
    Library.createSection(tabFrames.Games, "Game Scripts")
    Library.createSection(tabFrames.Scripts, "Fun Scripts")
    Library.createContentButton(tabFrames.Scripts, "Ball R6/R15", function()
    ...
    local keyBindContainer = Library.create("Frame", {
        Size = UDim2.new(1, 0, 0, 36),
        BackgroundTransparency = 1, Parent = tabFrames.Settings
    })
    Library.create("TextLabel", {
        Size = UDim2.new(0.45, 0, 1, 0), BackgroundTransparency = 1,
        Text = "Menu Toggle Key:", TextColor3 = Library.uiColor_TextColor,
        TextSize = 13, Font = Library.FONT,
    ...
    keyBindBtn.MouseButton1Click:Connect(function()
        ...
        connection = UserInputService.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.Keyboard then
                Library.currentToggleKey = input.KeyCode
                keyBindBtn.Text = Library.currentToggleKey.Name
    ...
    local colorSettings = {
        {"Main Window Color:", Library.formatColor(Library.uiColor_MainWindow), function(c) Library.uiColor_MainWindow = c end},
        {"Top Bar Color:", Library.formatColor(Library.uiColor_TopBar), function(c) Library.uiColor_TopBar = c end},
        {"Side Bar Color:", Library.formatColor(Library.uiColor_SideBar), function(c) Library.uiColor_SideBar = c end},
        {"Text Color:", Library.formatColor(Library.uiColor_TextColor), function(c) Library.uiColor_TextColor = c end},
        {"Button Color:", Library.formatColor(Library.uiColor_ButtonColor), function(c) Library.uiColor_ButtonColor = c end},
        {"TextBox Background Color:", Library.formatColor(Library.uiColor_TextBoxColor), function(c) Library.uiColor_TextBoxColor = c end},
    }
    for _, cfg in ipairs(colorSettings) do
        Library.createColorInput(tabFrames.Settings, cfg[1], function() return cfg[3] and Color3.new(0,0,0) end, cfg[3])
    end

    Library.createSection(tabFrames.Settings, "Background & Window")
    Library.createSlider(tabFrames.Settings, "Gui Opacity", 25, 100,
        function() return math.floor(Library.uiGuiOpacity * 100 + 0.5) end,
        function(v) Library.uiGuiOpacity = v / 100; Library.applyTheme() end,
        function(v) return v .. "%" end)
    Library.createSlider(tabFrames.Settings, "Blur", 0, 24,
        function() return Library.uiBlurSize end,
        function(v) Library.uiBlurSize = v; Library.updateBlur() end,
    ...
    Library.registerKeyListProvider("EmilyUi", function()
        local rows = {}
        if CharSettings.Speed ~= 16 then
            table.insert(rows, {"SPEED", tostring(CharSettings.Speed)})
        end
        if CharSettings.Jump ~= 50 then
            table.insert(rows, {"JUMP", tostring(CharSettings.Jump)})
        end
        if visToggles.Crosshair and visToggles.Crosshair:Get() then
            table.insert(rows, {"CROSSHAIR", "ON"})
        end
        if visToggles.Fullbright and visToggles.Fullbright:Get() then
            table.insert(rows, {"FULLBRIGHT", "ON"})
        end
        return rows
    end)
    ...
    local VisualsAPI = {
        ...
        Apply = function(d)
            ...
            if d.FOV then applyFOV(d.FOV) end
            if d.Crosshair ~= nil then setCrosshair(d.Crosshair) end
            if d.Fullbright ~= nil then setFullbright(d.Fullbright) end
        end,
        Reset = function()
    ...
    Library.VisualsAPI = VisualsAPI

    return {
        Tabs = Library.tabs,
        TabFrames = tabFrames,
        VisualsAPI = VisualsAPI,
    }
end
